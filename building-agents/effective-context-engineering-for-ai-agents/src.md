...
![Prompt engineering vs. context engineering](prompt-vs-context-engineering.png)
...
This attention scarcity stems from architectural constraints of LLMs. LLMs are based on the [transformer architecture](https://arxiv.org/abs/1706.03762), which enables every token to attend to every other token across the entire context. This results in n² pairwise relationships for n tokens.
...
![Calibrating the system prompt in the process of context engineering.](system-prompt-calibration.png)
...
As part of Anthropic's tooling releases, a memory tool in public beta on the Claude Developer Platform makes it easier to store and consult information outside the context window through a file-based system. This allows agents to build up knowledge bases over time, maintain project state across sessions, and reference previous work without keeping everything in context.
